...
//uk.ac.bbk.dcs.stypes.flink.FlinkRewritingSqlQ15
object FlinkRewritingSqlQ15 extends BaseFlinkTableRewriting {
    ...
    private def tableRewritingEvaluation(fileNumber: Int, jobName: String, tableEnv: TableEnvironment): Table = {
      lazy val p1 = tableEnv.sqlQuery(
        """SELECT DISTINCT p35_0.X0 AS x, p2_2.X1 AS y
          ...
          |          INNER JOIN A AS A_2 ON p3_0.X1 = A_2.X)""".stripMargin)
      p1
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length > 2) {
      FlinkRewritingSqlQ15.run(fileNumber, args(1), Try(args(2).toBoolean).getOrElse(false))
    } else if (args.length > 1) {
      FlinkRewritingSqlQ15.run(fileNumber, args(1))
    } else {
      FlinkRewritingSqlQ15.run(fileNumber)
    }
  }
}
