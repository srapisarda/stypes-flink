...
class FlinkQ27TableTest extends FunSpec with BaseFlinkTableTest {
  ...
  describe("Flink Talbe q27 ") {
    it("should execute p27 using file 6") {
      ...
      val r: Table = tableEnv.sqlQuery("select X as r_x, Y as r_y from R")
      ...
      val p27_1 = r.join(r).where("r_y=a_x").select("r_x as x7, a_y as x8")
          .join(r).where("x8=r_x").select("x7, r_y as x11")
          .join(r).where("x11=r_x").select("r_y as p27_x, x7 as p27_y")
      val p27_2 = p32.join(r).where("p32_y=r_x").select("p32_x as x7, r_y as x9")
          .join(r).where("x9=r_x").select("x7, r_y as x10")
      ...
      val p5_2= s.join(r).where("s_y=r_x").select("s_x as x0, r_y as x1")
        .join(r).where("x1=r_x").select("x0, r_y as x2")
      ...
      val p3_1 = a.join(s).where("a_y=s_x").select("a_x as x8, s_y as x9")
      ...
      lazy val p3_2 = a.join(r).where("a_y=r_x").select("a_x as x8, r_y as x11")
        .join(r).where("x11=r_x").select("r_y as p3_x, x8 as p3_y")
